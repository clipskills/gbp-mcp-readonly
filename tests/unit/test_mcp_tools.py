...
BANNED_IMPORTS = {"streamlit", "plotly", "fpdf", "google.generativeai", "gmb_app"}
SOURCE_ROOTS = (
    Path("src"),
    Path("data_fetcher.py"),
    Path("drive_helper.py"),
    ...
    for path in _python_files():
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Import):
                ...
                continue
            for name in names:
                assert name not in BANNED_IMPORTS, f"{path} imports {name}"
            for name in dotted:
                assert not name.startswith("gmb_app"), f"{path} imports {name}"
                assert name != "streamlit" and not name.startswith("streamlit."), (
                    f"{path} imports {name}"
                )
